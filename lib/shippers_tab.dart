...
class _ShippersTabState extends State<ShippersTab> {
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (filtered.isEmpty) return const Center(child: Text('No shippers found.'));
  ...
  Widget _locationCard({required String shipperId, required int index, required Map<String, dynamic> location}) {
            ...
            trailing: Row(
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () => _openAddEditLocationDialog(shipperId, index: index),
                ),
                IconButton(
                  tooltip: _canDelete ? 'Delete location' : 'No permission',
                  icon: const Icon(Icons.delete_outline),
                  onPressed: _canDelete ? () => _confirmDeleteLocation(shipperId, index) : null,
                ),
              ],
            ),
          ),
              ...
              trailing: Row(
                ...
                children: [
                  IconButton(
                    ...
                    icon: const Icon(Icons.edit),
                    onPressed: () => _openAddEditContactDialog(shipperId, index, contactIndex: c),
                  ),
                  IconButton(
                    tooltip: _canDelete ? 'Delete contact' : 'No permission',
                    icon: const Icon(Icons.delete_outline),
                    onPressed: _canDelete ? () => _confirmDeleteContact(shipperId, index, c) : null,
                  ),
                ],
              ),
            ),
  ...
  // ---------- Shipper ----------
  Future<void> _openAddEditShipperDialog({QueryDocumentSnapshot<Map<String, dynamic>>? doc}) async {
    final isEdit = doc != null;
    final data = isEdit ? (doc!.data()) : <String, dynamic>{};

    final name = TextEditingController(text: data['name'] ?? '');
    final mainAddress = TextEditingController(text: data['mainAddress'] ?? data['address'] ?? '');
    final mainPhone = TextEditingController(text: data['mainPhone'] ?? data['phone'] ?? '');
    final mainMobile = TextEditingController(text: data['mainMobile'] ?? '');
    final mainEmail = TextEditingController(text: data['mainEmail'] ?? data['email'] ?? '');
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  TextFormField(controller: hours, decoration: const InputDecoration(labelText: 'Hours of Operation')),
                  const SizedBox(height: 8),
                  TextFormField(controller: notes, decoration: const InputDecoration(labelText: 'Notes'), minLines: 1, maxLines: 4),
                ],
              ),
            ),
          ),
              ...
              onPressed: () async {
                ...
                try {
                  if (isEdit) {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Shipper updated.')));
                    }
                  } else {
                    await FirebaseFirestore.instance.collection('shippers').add({
                      ...payload,
                      'locations': [],
                      'createdAt': FieldValue.serverTimestamp(),
                    });
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Shipper added.')));
                    }
                  }
                } catch (e) {
                  if (mounted) {
  ...
  Future<void> _confirmDeleteShipper(QueryDocumentSnapshot<Map<String, dynamic>> d) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Shipper deleted.')));
      }
    } catch (e) {
      if (mounted) {
  ...
  // ---------- Location ----------
  Future<void> _openAddEditLocationDialog(String shipperId, {int? index}) async {
    final isEdit = index != null;
    final docRef = FirebaseFirestore.instance.collection('shippers').doc(shipperId);
  ...
  // ---------- Contact ----------
  Future<void> _openAddEditContactDialog(String shipperId, int locationIndex, {int? contactIndex}) async {
  ...
  Future<void> _confirmDeleteContact(String shipperId, int locationIndex, int contactIndex) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
